    ...
    <div class="overflow-x-auto">
        <table class="table-auto w-full text-left border-collapse border border-gray-200">
            ...
            <tbody>
                <?php if (!empty($Categories)): ?>
                    <?php foreach ($Categories as $category): ?>
                        <tr class="hover:bg-gray-100 border-b border-gray-200">
                            <!-- <td class="px-3 py-1"><?= $category['id'];?></td> -->
                            <td class="px-3 py-1"><?= $category['name']; ?></td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="3" class="px-3 py-1 text-center text-gray-500">Aucune catégorie trouvée.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<div class="mt-8">
    ...
    <div class="mt-8">
        <h2 class="text-2xl font-semibold text-gray-700 mb-4">Modifier la Catégorie</h2>
        <?php
        $categoryId = $_GET['id'];
        // var_dump($categoryId);
        ...
        if ($category): ?>
            <form action="../../index.php?action=editCategories" method="POST" class="space-y-4">
                <input type="hidden" name="id" value="<?= $category['id']; ?>">
                <div>
                    <label for="category_name" class="block text-gray-700">Nom de la catégorie</label>
                    <input type="text" name="name" id="name" value="<?= htmlspecialchars($category['name']); ?>" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                </div>
                <div>
        ...
        <?php else: ?>
            <p class="text-red-500">Catégorie non trouvée.</p>
        <?php endif; ?>
    </div>
<?php endif; ?>